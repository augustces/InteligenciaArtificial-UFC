...
def algGenetico(nenfermeiros, nturno,restricoes, tampopulacao, interacoes, elitismo, mutacao,var):
    ...
    for _ in range(interacoes):
        ...
        if elitismo > 0:
            ...
            # realiza o crossover
            novos_individuos = crossover(tampopulacao=tampopulacao, melhores_indices=melhores_indices, restricoes=restricoes, var=var, populacao=populacao)
...
def main():
    ...
    print("informe quais as restriçoes que devem compor a funçao fitness? (separe o numero das restriçoes por espaço em branco)") 
    entrada = input()
    ...
    cadeia, fit = algGenetico(nenfermeiros=nenfermeiros, nturno=nturnos, restricoes=restricoes, tampopulacao=tampopulacao, interacoes=interacoes, elitismo=elitismo, mutacao=mutacao, var=var)
    print(cadeia)
    print(fit)
